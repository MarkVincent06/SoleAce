                        ...
                        <tbody>
                           @foreach($subCategories as $index => $subCategory)
                              <tr>
                                 ...
                                 <td>
                                    {{$subCategory->status == '1' ? "Visible" : "Hidden"}}
                                 </td>
                                  <td>
                                    <a href="{{ route('admin.sub-category.edit', $subCategory->id) }}" class="btn btn-primary me-3">Edit</a>
                                    <form action="{{ route('admin.sub-category.destroy', $subCategory->id) }}" method="POST" style="display:inline;">
                                          @csrf
                                          @method('DELETE')
                                          <button type="submit" class="btn btn-danger delete-sub-category">Delete</button>
                                    </form>
                                 </td>
                              </tr>
                           @endforeach
